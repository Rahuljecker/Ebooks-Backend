...
export const BuySubscription = CatchAsyncError(async (req, res, next) => {
    ...
    const plan_id = process.env.RAZORPAY_PLAN_ID || "plan_LPSUEB8osuR6NX"
...
export const getRazorpayKey = CatchAsyncError(async (req, res, next) => {
    res.status(200).json({
        success: true,
        key: process.env.RAZORPY_KEY_ID,
    })
})
...
export const cancelsubscription = CatchAsyncError(async (req, res, next) => {
    ...
    res.status(200).json({
        success: true,
        message:
            refund ? "Subscription Cancelled!.You will receive payment in 7 days..Thank you " : "Subscription Cancelled!.You will not receieve  payment because Deadline is over..Thank you "
    })
})
